-- Fix: RETURNS TABLE output params named circle_ref / status / invitation_id
-- make unqualified (and sometimes aliased) column refs ambiguous in PL/pgSQL.
...
begin
  ...
  if v_existing_id is not null then
    ...
    into v_invitation_id, v_circle_ref, v_status, v_invite_token;
  else
    ...
    returning invitations.id, invitations.circle_ref, invitations.status, invitations.invite_token
    into v_invitation_id, v_circle_ref, v_status, v_invite_token;
  end if;
...
declare
  ...
  v_out_invitation_id uuid;
  v_out_circle_ref uuid;
  v_out_status text;
begin
  v_email := lower(trim(coalesce(p_email, '')));
  if v_email = '' then
  ...
  if not v_is_share_link then
    ...
    if v_duplicate_id is not null then
      ...
      into v_out_invitation_id, v_out_circle_ref, v_out_status;

      invitation_id := v_out_invitation_id;
      circle_ref := v_out_circle_ref;
      status := v_out_status;
      return next;
      return;
    end if;
...
begin
  ...
  if coalesce(v_current.is_share_link, false) then
    ...
    select f.invitation_id, f.circle_ref, f.status, f.invite_token
      into v_out_invitation_id, v_out_circle_ref, v_out_status, v_forked_token
...
begin
  ...
  if coalesce(v_invitation.is_share_link, false) then
    select i.invite_token, i.invitee_real_name, i.invitee_display_name, i.invitee_mobile,
           i.id, i.circle_ref, i.status
      into v_personal_token, v_invitee_real_name, v_invitee_display_name, v_invitee_mobile,
           v_out_invitation_id, v_out_circle_ref, v_out_status
    from public.invitations i
    ...
    where cm.circle_ref = v_out_circle_ref
      and cm.user_id = v_uid;

    if not found then
      insert into public.circle_members(circle_ref, user_id, role, status, joined_at)
      values (v_out_circle_ref, v_uid, 'member', 'active', now());
    end if;
  ...
  into v_out_invitation_id, v_out_circle_ref, v_out_status, v_invitee_real_name, v_invitee_display_name, v_invitee_mobile;
